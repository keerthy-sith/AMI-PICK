pipeline {
    ...
    environment {
        ...
        BACKEND_BUCKET        = "cf-templates-10ecz4bxqyxwf-us-east-1"
    }
    parameters {
        choice(
            name: 'Action',
...
def terraformInit() {
    sh("""
        ...
        terraform workspace select ${params.os.toLowerCase()} || terraform workspace new ${params.os.toLowerCase()}
    """)
}
def terraformPlan() {
...
def inspecValidation() {
    sh("""
        inspec exec Inspec/ -t aws:// --input workspace=${params.os}
    """)
}
